...
namespace AddressBook_LinQ
{
    public class ContactValidation
    {
        ...
        public bool CheckContactValidation(string fName, string lName, string address, string city, string state, string zipCode, string phoneNumber, string Email)
        {
            bool valid = false;
            if(ValidateFirstName(fName) && ValidateLastName(lName)&& ValidateAddress(address) && ValidateCitySate(city) && ValidateCitySate(state) && ValidateZipcode(zipCode) && ValidateMobileNumber(phoneNumber))
            {
                valid = true;
            }
            return valid;
